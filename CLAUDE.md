...
**New model investigated this session (2026-09-01): Qwen3.8-27B — TESTED in both bf16 and w8a8sc, neither adopted.** See the two dedicated sections below. **bf16** (`Huihui-Qwen3.8-27B-abliterated`): works cleanly at full 32768 context in `--enforce-eager` mode (ACLGraph crashes with `AclmdlRICaptureEnd`/507903 on this architecture), clean **10/10 pass@1** — the best accuracy of any Qwen3.5/3.6/3.8-family model tested here — but ~5x slower single-stream (5.94 tok/s) than current production. **w8a8sc** (`adeepv/Qwen3.8-27B-w8a8sc-Ascend310P`, non-abliterated, requires the mutable `nightly-releases-v0.25.1rc-310p-openeuler` image, not our pinned stable): same ACLGraph crash regardless of quantization (architecture-level, not bf16-specific), but +27% speed (7.55 tok/s) and a huge context win (131072+ confirmed, vs. bf16's 32768) — at the cost of a real, unexplained accuracy regression (**7/10 pass@1**, the first time quantization has hurt accuracy anywhere in this investigation). Neither variant is a strict win over current production; both remain on disk (`${MODELS_DIR}/Huihui-Qwen3.8-27B-abliterated`, `${MODELS_DIR}/adeepv-Qwen3.8-27B-w8a8sc-310P`) for any follow-up.
...
4. If Qwen3.8-27B is revisited: investigate whether the w8a8sc build's 7/10 pass@1 failures (`two_sum`, `anagram`, `roman`) are the known reasoning-non-termination pattern (fixable via sampling/`enable_thinking`) or genuine quantization-induced logic errors — not yet distinguished.
...
**Model used:** `huihui-ai/Huihui-Qwen3.8-27B-abliterated` (matches the huihui-ai abliterated-fork pattern used for every other model in this stack) — downloaded from HuggingFace via `hf download` with `hf_transfer` enabled, 52GB (18 safetensors shards, matches HF's file listing), to `${MODELS_DIR}/Huihui-Qwen3.8-27B-abliterated`. Its `generation_config.json` already ships `temperature: 0.2, repetition_penalty: 1.1` as defaults — the same reasoning-stability values this investigation had to manually discover and bake in via `--override-generation-config` for Qwen3.6 — suggesting Qwen may have addressed the reasoning-non-termination bug upstream in this release.
...
**This is the best pass@1 result of any Qwen3.5/3.6/3.8-family model tested on this hardware.** Both `Huihui-Qwen3.6-27B-abliterated` (dense) and `Huihui-Qwen3.6-35B-A3B-abliterated` (MoE, both bf16 and self-quantized w8a8) hit a real reasoning-non-termination bug on a meaningful fraction of tasks (30-40%, see "w8a8 production switch" section above) unless mitigated via `enable_thinking: false` or sampling tuning. This Qwen3.8-27B run hit **zero** such failures across the standard 10-task harness with no mitigation needed beyond what the checkpoint already ships by default (`temperature: 0.2, repetition_penalty: 1.1` baked into `generation_config.json`) — consistent with the theory that Qwen fixed this upstream for this release. Not yet confirmed against a larger prompt sample (only the standard 10-task harness was run) — the reasoning-non-termination bug was itself probabilistic (~30-40% hit rate) on the Qwen3.6 models, so a single clean 10/10 run is a strong signal but not full proof it's completely gone.
