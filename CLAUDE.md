...
**New model investigated this session (2026-09-01): Qwen3.8-27B (abliterated) — TESTED, works, not adopted.** See the dedicated "Qwen3.8-27B abliterated" section below. Works cleanly at full 32768 context in `--enforce-eager` mode (ACLGraph crashes with a `AclmdlRICaptureEnd`/507903 error on this architecture — same failure class surveyed but never confirmed in the `adeepv` section), and posted a clean 10/10 pass@1 on the standard bench harness — the best accuracy result of any Qwen3.5/3.6/3.8-family model tested here, with zero reasoning-non-termination failures (a bug that hit the Qwen3.6 models 30-40% of the time). Trade-off: ~5x slower single-stream (5.94 tok/s) than current production, since eager mode forfeits ACLGraph and this is dense-ish, not MoE. Model weights (52GB) remain on disk at `${MODELS_DIR}/Huihui-Qwen3.8-27B-abliterated` for follow-up testing (a `w8a8-310p` pre-quantized variant exists and hasn't been tried yet — could plausibly close the speed gap the same way w8a8 did for Qwen3.6).
...
4. Test the `w8a8-310p` pre-quantized variant of Qwen3.8-27B — could unlock ACLGraph (untested whether the 507903 capture-end crash is bf16-cast-specific) and close the speed gap to current production, mirroring the w8a8 win pattern already seen with Qwen3.6.
