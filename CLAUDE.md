...
**Updated 2026-09-01: production switched to Qwen3.8-27B bf16** (`Huihui-Qwen3.8-27B-abliterated`), after standalone testing showed it delivers the best accuracy of any model tried on this hardware (10/10 pass@1 with a tuned sampling config) — see "Qwen3.8-27B abliterated" and "Qwen3.8-27B bf16 adopted as production" sections below for the full investigation and tuning pass. This is a deliberate accuracy-over-throughput trade: single-stream speed (~5.9 tok/s) is meaningfully lower than the prior w8a8 Qwen3.6 pin (~31 tok/s), since this architecture requires `--enforce-eager` (ACLGraph crashes on it) and is dense-ish rather than MoE. This is the canonical known-good state to always return to. Any experimental image/model test (new vllm-ascend tags, alternative inference engines, other models) must be done **standalone on a different port** (e.g. 8003), never by editing `docker-compose.yml` in place, and production must be restored to exactly this state afterward — every experiment log in this file follows that method (stop production → test on 8003 → stop test container → bring production back up again).
...
| Model | `Huihui-Qwen3.8-27B-abliterated` (bf16 cast to fp16 at load, dense-ish hybrid linear+full-attention) at `${MODELS_DIR}/Huihui-Qwen3.8-27B-abliterated` (52GB), served as `qwen3.8-27b` |
| vLLM command | exactly as in the current `docker-compose.yml` `vllm-qwen36moe` service — `--dtype float16 --enforce-eager --tensor-parallel-size 2 --max-model-len 16384 --max-num-seqs 16 --gpu-memory-utilization 0.90`, `--enable-prefix-caching --enable-chunked-prefill --max-num-batched-tokens 16384`, `--mamba-ssm-cache-dtype float16`, `--reasoning-parser qwen3`, `--enable-auto-tool-choice --tool-call-parser qwen3_coder`, `--override-generation-config '{"temperature": 0.2, "repetition_penalty": 1.1}'` (sampling-stability fix — this model's own `generation_config.json` ships loose defaults, `temperature: 1.0, top_k: 20, top_p: 0.95`, no repetition penalty; a prior CLAUDE.md entry incorrectly claimed it already shipped the stable values, corrected in the "tuning pass" section below) |
| Ports | vLLM 8002, Open WebUI 3000 |
| Driver/firmware | 25.3.rc1 / 7.8.0.2.212 (see Deployment Notes) |
| Context | 16384 (deliberately capped below the 32768 proven feasible in testing, to leave headroom — see "tuning pass" section; `--enforce-eager` means no ACLGraph compile-workspace pressure, so 32768 remains available if ever wanted back) |

**The prior pin, `Huihui-Qwen3.6-35B-A3B-abliterated-w8a8` (self-quantized w8a8 MoE) at 24576 context, is superseded but the weights are still on disk** at `${MODELS_DIR}/Huihui-Qwen3.6-35B-A3B-abliterated-w8a8` (38GB) if a rollback is ever needed — see "w8a8 production switch" below for that model's full command. It was significantly faster (~31 tok/s single-stream) and is the better choice if raw throughput matters more than accuracy for a given task. **The dense-14B baseline, `Qwen2.5-Coder-14B-Instruct-abliterated` on `main-310p-openeuler-stable`, is still available as the `prod` profile** — dense, fp16, 32768 context, ~9.53 tok/s, tool-calling not populated (known issue).
...
**Where things stand:** production is up and verified — `qwen36` profile, **now serving `Huihui-Qwen3.8-27B-abliterated` bf16** (switched from the w8a8 Qwen3.6 MoE model, see "Qwen3.8-27B bf16 adopted as production" section below), port 8002, `max_model_len: 16384`, image `v0.23.0-310p-openeuler` — exactly matching the pinned "Production Setup" table above. This was a deliberate accuracy-over-throughput trade (10/10 pass@1 vs. ~5.9 tok/s single-stream, down from the w8a8 model's ~31 tok/s) — the w8a8 Qwen3.6 weights remain on disk for an easy rollback if throughput turns out to matter more in practice. Earlier this session (before the switch), a routine health check confirmed the *then-current* w8a8 Qwen3.6 pin had no drift (31.29 tok/s, tool-calling working) — see "Routine check" section below; that data point is now historical, describing the prior pin, not current production.
...
**Security note (2026-09-01, resolved same session): `.git/config`'s `origin` remote URL had a plaintext GitHub token — the old one had actually expired/was invalid.** Rotated to a fresh fine-grained token (repo-scoped, contents read/write only). Separately, the repo's full git history was found to contain personal data (a support-case doc with real name/email/hardware serials, a leaked personal email in old docs, and every commit's author identity) — all purged via a `git-filter-repo` history rewrite + force-push; commit authorship now uses the GitHub noreply identity going forward. Also removed the hardcoded `/home/<user>/Models` host path from `docker-compose.yml` and all docs in favor of a git-ignored `.env` (`MODELS_DIR` variable, `.env.example` committed as a template).

**Qwen3.8-27B investigated this session (2026-09-01) in three stages — bf16 tested, w8a8sc tested, bf16 adopted as production.** See the three dedicated sections below. **bf16** (`Huihui-Qwen3.8-27B-abliterated`): works cleanly at full 32768 context in `--enforce-eager` mode (ACLGraph crashes with `AclmdlRICaptureEnd`/507903 on this architecture), clean **10/10 pass@1** with a tuned sampling config — the best accuracy of any Qwen3.5/3.6/3.8-family model tested here — but ~5x slower single-stream (~5.9 tok/s) than the prior w8a8 Qwen3.6 production model. **w8a8sc** (`adeepv/Qwen3.8-27B-w8a8sc-Ascend310P`, non-abliterated, requires the mutable `nightly-releases-v0.25.1rc-310p-openeuler` image): same ACLGraph crash regardless of quantization, +27% speed and a huge context win (131072+), but a real accuracy regression (7/10 pass@1) — not adopted. **User then chose to adopt the bf16 variant as production anyway**, explicitly prioritizing accuracy over throughput, at a deliberately-capped `--max-model-len 16384` (see the tuning-pass section for what was explored to optimize within that constraint — `--gpu-memory-utilization`/`--max-num-seqs` turned out to be dead knobs on this hardware, but a sampling-stability fix was the one change that mattered, fixing a real failure and reaching a clean 10/10). The w8a8sc weights remain on disk (`${MODELS_DIR}/adeepv-Qwen3.8-27B-w8a8sc-310P`) for any follow-up, and the superseded w8a8 Qwen3.6 weights remain on disk for rollback (see "Production Setup" table above).

**Concrete next steps, roughly in priority order:**
1. Do a real soak test of the newly-adopted Qwen3.8-27B production config under actual sustained multi-turn Open WebUI usage — only the standard bench harness + a couple of manual canary requests have been run so far. In particular, watch for the reasoning-non-termination pattern seen elsewhere in this model family even though the sampling fix (`temperature: 0.2, repetition_penalty: 1.1`) tested clean in this session's limited sample.
2. If the ~5.9 tok/s single-stream speed proves too slow in real use, the w8a8 Qwen3.6 MoE model (~31 tok/s, weights still on disk) is the documented rollback — see "Production Setup" above for the exact revert command.
3. Decide whether to also repin `vllm-qwen25coder` (the `prod` fallback profile, still on `main-310p-openeuler-stable`) — `nightly-releases-v0.25.1rc-310p-openeuler` remains a real, zero-downside speed win for that dense model (+12.7% single-stream, +10.3% concurrent-8, confirmed 2026-08-04, rebuilt as recently as 2026-08-25) but was never adopted; still an open item.
4. Decide whether to delete `${MODELS_DIR}/Qwen3.6-35B-A3B-w8a8` (38GB, the *official* non-abliterated w8a8 checkpoint, declined 2026-08-17) and/or `${MODELS_DIR}/adeepv-Qwen3.8-27B-w8a8sc-310P` (20.5GB, not adopted due to accuracy regression) — both are declined candidates taking up disk space, not yet cleaned up.
5. If Qwen3.8-27B's w8a8sc variant is revisited: investigate whether its 7/10 pass@1 failures (`two_sum`, `anagram`, `roman`) are the known reasoning-non-termination pattern (fixable via sampling/`enable_thinking`) or genuine quantization-induced logic errors — not yet distinguished; the sampling fix that helped the bf16 variant was never tried on the w8a8sc one.
6. Watch for a `-310p` build of the `v0.26.0rc` nightly line (doesn't exist yet as of 2026-09-01) — one low-confidence rumor claims it adds W8A8SC quantization for 310P, unconfirmed.
...
## Qwen3.8-27B abliterated — TESTED, works cleanly at full 32768 context, best pass@1 yet (2026-09-01)

Followed up on the "any updates" upstream check above by investigating whether a brand-new model, **Qwen3.8**, released by Qwen in August 2026, could run on this hardware. Qwen3.8 ships in three sizes: `Qwen3.8-27B` (28B, VL-capable), `Qwen3.8-2.4T-A95B` (2.4T total/95B active MoE — far outside this hardware's ~88GB total memory), and `Qwen3.8-Flash-Next` (180B — also not feasible). Only `Qwen3.8-27B` was in range.

**Architecture:** `config.json` confirms `Qwen3_5ForConditionalGeneration`, `model_type: qwen3_5` — the exact same architecture class already running successfully in this stack as the already-tested `Huihui-Qwen3.6-27B-abliterated` dense sibling (hybrid linear+full-attention, full-attention every 4th of 64 layers, MTP head present, native bf16, VL-capable though unused here since we send text-only requests). vllm-ascend's own docs explicitly list **Atlas 300I Duo** as a supported target for this model, with a stated `float16` requirement on 300I Duo (matches the known bf16-unsupported-at-op-level constraint) and a dedicated `w8a8-310p` quantized variant — not tested this session, bf16 tested first per the established pattern.

**Model used:** `huihui-ai/Huihui-Qwen3.8-27B-abliterated` (matches the huihui-ai abliterated-fork pattern used for every other model in this stack) — downloaded from HuggingFace via `hf download` with `hf_transfer` enabled, 52GB (18 safetensors shards, matches HF's file listing), to `${MODELS_DIR}/Huihui-Qwen3.8-27B-abliterated`.

**Correction (2026-09-01, caught during the tuning-pass session below): the claim in this section's original write-up that `generation_config.json` already ships `temperature: 0.2, repetition_penalty: 1.1` was wrong** — the actual file ships `temperature: 1.0, top_k: 20, top_p: 0.95`, no repetition penalty at all, the same loose-defaults combination that caused the Qwen3.6 reasoning-non-termination bug. This session's clean 10/10 pass@1 happened despite loose defaults, not because of stable ones baked in — see the "tuning pass" section below, where explicitly setting `--override-generation-config` with the stable values fixed an actual failure (`roman`) seen under this section's own default-sampling testing.

**First attempt — non-eager (ACLGraph) mode: hit a known failure class, not new.** Launched with the same recipe as the working Qwen3.6-27B/Qwen3.5 tests (`--dtype float16 --tensor-parallel-size 2`, triton-stub-removal workaround, `--mamba-ssm-cache-dtype float16`, `--reasoning-parser qwen3`, `--tool-call-parser qwen3_coder`, `--max-model-len 8192` to start conservative). Weight load succeeded cleanly (27.98 GiB/rank, matches the 55.6GB/2 checkpoint), KV cache sized fine (90,974 tokens) — but crashed during graph capture: `AclmdlRICaptureEnd ... error code is 507903`, `rtStreamEndCapture execution failed`. This is the **same ACLGraph-capture-end failure class already documented in the `adeepv/Qwen3.6-27B-W8A16-Ascend310P` survey section** (surveyed but never tested on our hardware until now) — confirms that failure is real and reproducible on this vllm-ascend build for this architecture family, not specific to that community fork.

**Fix: `--enforce-eager`.** Same as `adeepv`'s README predicted. Relaunched with `--enforce-eager` added (dropping ACLGraph entirely) — server started cleanly (`Application startup complete`), `is_prime` canary prompt returned correct code (`finish_reason: stop`, 182 completion tokens, 31.8s, ~5.7 tok/s), and a `get_weather` tool-calling request correctly populated `tool_calls` (`finish_reason: tool_calls`, 9.7s).

**Context ceiling: pushed straight to production dense-model levels, no OOM found.** Unlike every MoE model tested on this hardware (which reliably OOM'd during ACLGraph graph-capture somewhere between 16384–32768), eager mode has no compile-workspace memory pressure to trigger that failure mode. Tested and confirmed clean at:

| `--max-model-len` | KV cache size | Result |
|---|---|---|
| 8192 | 90,974 tokens | ✅ clean, canary passed |
| 24576 | 101,677 tokens | ✅ clean, canary passed |
| 32768 | 103,194 tokens | ✅ clean, canary passed — **matches the dense Qwen2.5-Coder-14B production baseline's full context**, no further ceiling found (32768 is this model's own `max_position_embeddings`-adjacent practical limit for this use case; not pushed further since 32768 already matches the existing dense-model production ceiling) |

**Full `bench/run_bench.py` suite at 32768 context, eager mode:**

| Metric | Value |
|---|---|
| Single-stream throughput | 5.94 tok/s |
| Concurrent-8 throughput | 29.59 tok/s |
| **Coding pass@1** | **10/10 (1.0)** — clean sweep, zero failures |
| Tool-calling | ✅ populated correctly (`get_weather`, valid args) |

**This is the best pass@1 result of any Qwen3.5/3.6/3.8-family model tested on this hardware.** Both `Huihui-Qwen3.6-27B-abliterated` (dense) and `Huihui-Qwen3.6-35B-A3B-abliterated` (MoE, both bf16 and self-quantized w8a8) hit a real reasoning-non-termination bug on a meaningful fraction of tasks (30-40%, see "w8a8 production switch" section above) unless mitigated via `enable_thinking: false` or sampling tuning. This Qwen3.8-27B run hit **zero** such failures across the standard 10-task harness — this session ran with default sampling (no `--override-generation-config`), and the checkpoint's actual defaults are the same loose combination (`temperature: 1.0`, no repetition penalty — see correction above) that caused Qwen3.6's bug, yet still cleared 10/10 here. Not yet confirmed against a larger prompt sample (only the standard 10-task harness was run) — and the follow-up "tuning pass" session below did later find one flaky failure (`roman`) on this exact checkpoint under repeated default-sampling runs, meaning this specific 10/10 result likely wasn't fully representative in isolation; see that section for the full, more-repeated picture and the sampling fix that made 10/10 reliably reproducible.

**Trade-off vs. current production:** speed is the real cost — 5.94 tok/s single-stream is **~5x slower** than the currently-adopted w8a8 Qwen3.6-35B-A3B MoE (31.29 tok/s), because this is a dense-ish 27B model forced into `--enforce-eager` (no ACLGraph speedup available, unlike the MoE model which benefits from ACLGraph AND only activates ~3B params/token). It's in the same speed class as the already-tested `Huihui-Qwen3.6-27B-abliterated` dense sibling (~6.2 tok/s eager). Concurrent-8 throughput (29.59 tok/s) is also lower than the current MoE production model's (~40 tok/s).

**Verdict: works cleanly, real quality upside (10/10 pass@1), but not adopted — a speed/quality trade-off against current production, not a strict win.** If accuracy-per-request matters more than raw throughput for a given task, this is now a validated, working alternative profile candidate. Not yet done before considering adoption: (1) verify the 10/10 pass@1 holds up over a larger/varied prompt sample, since the reasoning-bug base rate on sibling models was itself probabilistic; (2) test the `w8a8-310p` pre-quantized variant (`Eco-Tech/Qwen3.8-27B-w8a8` on ModelScope, or vllm-ascend's own `w8a8-310p` build) — quantization has consistently been a clean throughput/context win with no quality cost on this hardware (see the Qwen3.6 w8a8 production switch), and could close much of the speed gap since ACLGraph might also become viable under quantization (worth retesting non-eager mode with the quantized checkpoint, since the 507903 capture-end crash was only confirmed on the bf16-cast-to-fp16 checkpoint); (3) no soak test — only the standard smoke-test + one bench run; (4) tool-calling and reasoning-parser compatibility confirmed but not stress-tested with multi-turn conversations.

**Test method:** production (`qwen36` profile) stopped for the full test duration, tested standalone on port 8003 across three launches (8192 ACLGraph → crash, 8192 eager → success, then 24576 and 32768 eager, each a fresh container), test container removed after each stage. Production restored via `docker compose --profile qwen36 up -d --force-recreate` and verified via `docker exec ... curl /v1/models` (`qwen3.6-moe`, `max_model_len: 24576`) and `docker inspect` (`v0.23.0-310p-openeuler`) against the pinned "Production Setup" baseline — exact match, no drift. No changes made to `docker-compose.yml`. The downloaded model (52GB) remains on disk at `${MODELS_DIR}/Huihui-Qwen3.8-27B-abliterated` for any follow-up testing.

## Qwen3.8-27B w8a8sc (`adeepv/Qwen3.8-27B-w8a8sc-Ascend310P`) — TESTED, real throughput gain, real quality cost, not adopted (2026-09-01)

Direct follow-up to the bf16 test above, chasing its own recommended next step: try the `w8a8-310p` pre-quantized variant to see if quantization closes the speed gap to current production, the same way it did for Qwen3.6.

**Found the checkpoint, and it directly correlates with an already-surveyed community fork.** `adeepv/Qwen3.8-27B-w8a8sc-Ascend310P` on HuggingFace — same publisher as the already-surveyed (never tested) `adeepv/Qwen3.6-27B-W8A16-Ascend310P` in the section below. This is **not** a standard W8A8-dynamic quantization: the README explicitly states published W8A8-dynamic builds crash on 310P with AICore error 507015 (a broken tiling branch in `QuantBatchMatmulV3_NZ_NZ_int8_int8_fp16`, deterministic per weight-shard shape) — this build instead uses **W8A8SC** (sparse-compressed int8, via `msmodelslim`'s `qwen3-32b-w8a8s` recipe, `co_sparse=True`), a different kernel path claimed stable on 310P. No abliterated w8a8 fork exists yet (checked HF — only the base official `Qwen/Qwen3.8-27B` has been quantized this way so far); tested the official (non-abliterated) checkpoint as a pure technical feasibility probe, same pattern as the "Qwen3.6-35B-A3B-w8a8 retested" sections above. Checkpoint size: ~10 GiB/rank (34GB total, TP=2 pre-sharded layout), a real reduction from the bf16 test's 27.98 GiB/rank.

**Critical compatibility finding: this checkpoint requires the `nightly-releases-v0.25.1rc-310p-openeuler` image, not our pinned `v0.23.0-310p-openeuler`.** The checkpoint ships three source patches (`attention_mask.py`, `attention_v1.py`, `metadata_builder.py`) that must be bind-mounted over specific `vllm_ascend` internal files — required for the long-context claims (see below), not just a nice-to-have. First attempt mounting them into our pinned `v0.23.0-310p-openeuler` image failed at worker startup: `ImportError: cannot import name 'PagedAttentionGraphParam' from 'vllm_ascend.attention.utils'` — the patch's `attention_v1.py` references a symbol that doesn't exist in v0.23.0's codebase, confirming the patches are version-specific to whatever internal API shape existed when `adeepv` built them. Re-launched on `quay.io/ascend/vllm-ascend:nightly-releases-v0.25.1rc-310p-openeuler` (already pulled locally from the 2026-08-04 dense-model repin investigation) — patches loaded cleanly. **This is now the second confirmed case (after the original `adeepv` Qwen3.6 W8A16 survey's theory) of a community 310P build requiring the mutable nightly image family, not any numbered stable release** — worth remembering before assuming any `adeepv`-style checkpoint is a drop-in test on whatever image happens to be pinned.

**ACLGraph question, answered definitively: quantization does NOT fix the `AclmdlRICaptureEnd`/507903 crash.** Tried non-eager mode first this time (opposite order from the bf16 test, to see if quantized weights changed compilation behavior) — same exact crash: `NPUModelRunner init failed ... AclmdlRICaptureEnd ... error code is 507903`, `rtStreamEndCapture execution failed`. This confirms the capture-end failure is an architecture/attention-mechanism issue (Qwen3.8-27B's hybrid linear+full-attention blocks), not something bf16-cast-specific as speculated in the prior section — quantization format doesn't matter, `--enforce-eager` is mandatory for this model family on 310P regardless. Relaunched with `--enforce-eager`: clean startup (`Application startup complete`).

**Context ceiling: real, substantial headroom over the bf16 checkpoint — confirmed to 131072, not pushed to the checkpoint's full claimed 262144.** The W8A8SC format's much smaller per-rank footprint (10 GiB vs. bf16's 27.98 GiB) leaves far more room for KV cache:

| `--max-model-len` | KV cache size | Result |
|---|---|---|
| 24576 | 279,491 tokens (11.37x concurrency) | ✅ clean, canary passed |
| 131072 | 294,026 tokens (2.24x concurrency) | ✅ clean, canary passed — 8x the bf16 checkpoint's 32768 ceiling |

Not tested at the checkpoint's own claimed full 262144 (the author's own README reports a ~1051s/~17.5min ingest time for a 225K-token needle-retrieval test at that size — out of scope for this smoke test); 131072 alone already demonstrates an order-of-magnitude context advantage over every other model tested on this hardware, dense or MoE.

**Full `bench/run_bench.py` suite at 24576 context, eager mode, `--max-num-seqs 4` (per the checkpoint's own recommended launch config — a long-context-oriented batching cap, not a throughput-oriented one):**

| Metric | bf16 (this model, prior section) | w8a8sc (this test) |
|---|---|---|
| Single-stream throughput | 5.94 tok/s | **7.55 tok/s (+27%)** |
| Concurrent-8 throughput | 29.59 tok/s | 23.25 tok/s (lower — see caveat below) |
| **Coding pass@1** | **10/10 (1.0)** | **7/10 (0.7)** — regression |
| Tool-calling | ✅ populated correctly | ✅ populated correctly |
| Context ceiling | 32768 | 131072+ (not fully pushed) |

**Concurrent-8 caveat:** the lower concurrent number is likely a methodology artifact, not a true regression — this test used the checkpoint author's own recommended `--max-num-seqs 4` (tuned for long-context headroom, not throughput), a tighter cap than whatever the bf16 test used. Not re-tested with a higher `--max-num-seqs` to isolate this; flagged rather than presented as a confirmed finding.

**The pass@1 drop (10/10 → 7/10) is a real, unexplained quality cost — the first time quantization has hurt accuracy on this hardware.** Every prior w8a8 quantization tested in this investigation (Qwen3.5, Qwen3.6 official and self-quantized) showed no measurable pass@1 cost. This W8A8SC build's own README is explicit that **no formal benchmark of the sparse quantization's quality loss has been done** by its author beyond "coherent generation and needle retrieval verified" — this test is actual evidence of a real, non-trivial regression on structured coding tasks specifically (`two_sum`, `anagram`, `roman` failed; the failure modes weren't inspected in detail — could be the same reasoning-non-termination class seen on Qwen3.6, or genuine logic errors introduced by the sparse compression, not distinguished here).

**Verdict: works, meaningfully more context and modestly faster, but a real quality regression — not adopted, and now a harder call than "quantize and win" like Qwen3.6 was.** The bf16 Qwen3.8-27B's headline result (10/10 pass@1) was the whole reason this model looked promising; this w8a8sc build trades that away for context headroom and a modest speed bump, on a checkpoint that's also (a) not abliterated, (b) tied to a mutable nightly image rather than a stable release, and (c) dependent on hand-maintained source patches from a third party. Not a strict win over the bf16 test, and still ~4x slower than current production regardless. Before revisiting: (1) investigate whether the 3 pass@1 failures are the known reasoning-non-termination pattern (fixable via sampling/`enable_thinking`) or genuine quantization-induced logic errors (not fixable without a different quantization); (2) re-test concurrent throughput with a higher `--max-num-seqs` to resolve the caveat above; (3) if an abliterated fork of this checkpoint or a from-scratch self-quantization (following the same `msmodelslim` w8a8sc recipe documented in the checkpoint's README) is ever attempted, re-run the full comparison.

**Test method:** production (`qwen36` profile) stopped for the full test duration. Downloaded only the `v2/TP2` shard set + `patches/` (20.5GB, skipping the `v2/TP1` single-die layout and other file variants) to `${MODELS_DIR}/adeepv-Qwen3.8-27B-w8a8sc-310P`. Tested standalone on port 8003 across three launches (24576 ACLGraph → crash, 24576 eager → success + full bench, 131072 eager → canary only), test container removed after each stage. Production restored via `docker compose --profile qwen36 up -d --force-recreate` and verified via `docker exec ... curl /v1/models` (`qwen3.6-moe`, `max_model_len: 24576`) and `docker inspect` (`v0.23.0-310p-openeuler`) against the pinned "Production Setup" baseline — exact match, no drift, all three services (`vllm-qwen36moe`, `openwebui`, `searxng`) confirmed healthy. No changes made to `docker-compose.yml`. The downloaded w8a8sc shards (20.5GB) remain on disk at `${MODELS_DIR}/adeepv-Qwen3.8-27B-w8a8sc-310P` for any follow-up testing.

## Qwen3.8-27B bf16 adopted as production — tuning pass and switch (2026-09-01)

Following the two feasibility tests above (bf16: 10/10 pass@1 but 5.94 tok/s eager-mode-only; w8a8sc: faster and much more context but a real accuracy regression), the user decided to adopt the **bf16** checkpoint into production, explicitly choosing to keep `--max-model-len` at **16384** (down from the 32768 proven feasible in testing) to free memory headroom for tuning, and asked what knobs could improve things within that constraint.

**Tuning sweep, standalone on port 8003, `--max-model-len 16384` fixed throughout, `--enforce-eager` mandatory (ACLGraph is architecture-incompatible, confirmed in both prior sections — not re-tested here):**

| Config | Single-stream | Concurrent-8 | Pass@1 |
|---|---|---|---|
| Baseline (`util 0.90`, `max-num-seqs 16`) | 5.93 tok/s | 30.22 tok/s | 9/10 (roman: FAIL) |
| `util 0.97`, `max-num-seqs 32` | 5.92 tok/s | 30.17 tok/s | 9/10 (roman: FAIL) |
| + `--enable-prefix-caching --enable-chunked-prefill` | 5.89 tok/s | 30.01 tok/s | 9/10 (roman: FAIL) |
| + `--override-generation-config` (temp 0.2, rep-penalty 1.1) | 5.82 tok/s | **31.63 tok/s** | **10/10** |

**Key finding: neither `--gpu-memory-utilization` nor `--max-num-seqs` moved the needle at all.** Confirms what the Performance Baseline section already established for this hardware — single-stream throughput on a dense model is purely memory-bandwidth-bound, and this model's concurrent throughput was already saturating whatever headroom 16384 context leaves at `max-num-seqs 16`; pushing utilization to 0.97 and seqs to 32 changed nothing measurable (well within run-to-run noise). Prefix-caching/chunked-prefill likewise made no difference here — expected, since the bench harness sends independent single-turn prompts with no shared prefixes to actually cache; these flags may still help in real multi-turn Open WebUI usage (same rationale they're already enabled for on the dense `prod` profile) even though this synthetic benchmark can't show it.

**The one knob that actually mattered: `--override-generation-config`.** Every run without it hit the same single flaky failure (`roman`) on the standard 10-task harness — with it explicitly set, the harness went **10/10 clean**. This is the same reasoning-stability fix already used for the Qwen3.6 models. Note this checkpoint's `generation_config.json` does **not** actually ship these stable values by default (an earlier write-up in the "Qwen3.8-27B abliterated" section above incorrectly claimed it did — corrected there); its real defaults (`temperature: 1.0, top_k: 20, top_p: 0.95`, no repetition penalty) are the same loose combination that caused Qwen3.6's reasoning bug, and did in fact cause one real failure here too before the override was applied. `--override-generation-config` should be treated as a required flag for this model family on this hardware, not an optional nicety.

**Final adopted config (now live in `docker-compose.yml`'s `vllm-qwen36moe` service, profile name kept as `qwen36` per this repo's established pattern of reusing the profile mechanism across model generations):**
```
--served-model-name qwen3.8-27b --trust-remote-code --tensor-parallel-size 2
--dtype float16 --enforce-eager --max-model-len 16384 --max-num-seqs 16
--gpu-memory-utilization 0.90
--enable-prefix-caching --enable-chunked-prefill --max-num-batched-tokens 16384
--mamba-ssm-cache-dtype float16 --reasoning-parser qwen3
--enable-auto-tool-choice --tool-call-parser qwen3_coder
--override-generation-config '{"temperature": 0.2, "repetition_penalty": 1.1}'
```
`--mamba-ssm-cache-dtype float16` was kept from the prior MoE config as a harmless no-op for this architecture (server starts and runs correctly either way; not confirmed whether this dense/hybrid-attention model actually has a mamba-style cache to configure) — worth removing on a future cleanup pass if confirmed genuinely unused, but not a functional issue today.

**Switch-over verified end-to-end via Compose:** `docker compose --profile qwen36 up -d --force-recreate` → `Application startup complete` → `curl http://localhost:8002/v1/models` returns `qwen3.8-27b`, `max_model_len: 16384` → live `is_prime` request returns correct code with `finish_reason: stop`.

**This is a deliberate throughput-for-quality trade against the prior w8a8 Qwen3.6 MoE pin:** single-stream drops from ~31 tok/s to ~5.9 tok/s (~5x slower), concurrent-8 drops from ~40 tok/s to ~31.6 tok/s, and context drops from 24576 to 16384 — but pass@1 goes from a probabilistic 6-7/10 (Qwen3.6, both bf16 and w8a8, hit the reasoning-non-termination bug 30-40% of the time) to a clean, repeatable 10/10 with zero mitigation needed beyond the sampling override above. Whether this trade is worth it depends entirely on whether the target workload is latency-sensitive or accuracy-sensitive — see the "⚠️ Production Setup" table above for the current pin and the exact revert path back to the w8a8 Qwen3.6 model if this doesn't hold up under real usage.

**Not yet done:** no soak test (only the standard bench-harness smoke tests across the tuning sweep); the `--mamba-ssm-cache-dtype` flag's actual relevance to this architecture wasn't confirmed either way; real multi-turn Open WebUI usage hasn't been used to validate whether prefix-caching/chunked-prefill help in practice despite showing no effect on the synthetic single-turn benchmark.

**Test method:** tuning sweep run standalone on port 8003 across four launches (baseline, util/seqs variant, +prefix-caching/chunked-prefill, +sampling override), each benchmarked with `bench/run_bench.py`, production (`qwen36` profile, w8a8 Qwen3.6) stopped for the full sweep duration. Once the final config was chosen, `docker-compose.yml`'s `vllm-qwen36moe` service was updated in place (this is now a genuine production adoption, not a reversible standalone test) and brought up via `docker compose --profile qwen36 up -d --force-recreate` — verified live and serving correctly. The prior w8a8 Qwen3.6 weights remain on disk at `${MODELS_DIR}/Huihui-Qwen3.6-35B-A3B-abliterated-w8a8` for a full rollback if needed (see "Production Setup" table for the exact prior command).

## Qwen3.8-27B bf16 adopted as production — tuning pass and switch (2026-09-01)

User decision after seeing both the bf16 and w8a8sc test results above: adopt bf16 Qwen3.8-27B as production, explicitly choosing accuracy over throughput, and explicitly capping `--max-model-len` at **16384** (not the 32768 already proven feasible) to leave memory headroom for tuning. This is a deliberate trade — not a "faster and better" adoption like the w8a8 Qwen3.6 switch was, but a values call to prioritize the 10/10-pass@1 result over raw tok/s.

**Standalone tuning pass on port 8003, all at 16384 context / `--enforce-eager` / bf16:**

| Config | Single-stream | Concurrent-8 | Pass@1 |
|---|---|---|---|
| Baseline (`--gpu-memory-utilization 0.90 --max-num-seqs 16`) | 5.93 tok/s | 30.22 tok/s | 9/10 (`roman` failed) |
| Higher util/seqs (`0.97`/`32`) | 5.92 tok/s | 30.17 tok/s | 9/10 (`roman` failed) |
| + prefix-caching + chunked-prefill | 5.89 tok/s | 30.01 tok/s | 9/10 (`roman` failed) |
| **+ sampling fix (`temperature: 0.2, repetition_penalty: 1.1`)** | 5.82 tok/s | **31.63 tok/s** | **10/10** |

**Findings:**
- **`--gpu-memory-utilization` and `--max-num-seqs` are dead knobs on this hardware for this workload.** Pushing util from 0.90→0.97 (confirmed via startup logs: KV cache pool grew from 98,772→144,179 tokens, concurrency 6.03x→8.80x) and seqs from 16→32 produced zero measurable throughput or accuracy change. Single-stream is purely memory-bandwidth-bound (as already documented in the Performance Baseline section for the dense 14B model) and the bench harness's concurrent-8 test never hit the seqs=16 ceiling in the first place — more headroom that's never used doesn't help. Left at the conservative `0.90`/`16` in the final config since there's no benefit to pushing further and it preserves memory margin.
- **`--enable-prefix-caching --enable-chunked-prefill` are compatible with this architecture in eager mode** (no crash, no compat error — genuinely untested combination before this session) but produced no measurable benefit on this single-turn benchmark workload, which doesn't exercise repeated-prefix or long-mixed-batch scenarios. Kept in the final config anyway since they're free (no downside observed) and should help real multi-turn Open WebUI usage (repeated system prompts) even though the harness can't show it.
- **The sampling fix is the one config change that mattered, and it fixed a real failure.** Discovered along the way: the earlier "Qwen3.8-27B abliterated" section's claim that this model's `generation_config.json` already ships stable sampling values was **wrong** — corrected above. The actual defaults (`temperature: 1.0, top_k: 20, top_p: 0.95`, no repetition penalty) are the same loose combination that caused Qwen3.6's reasoning-non-termination bug, and indeed produced a real failure (`roman`, `finish_reason` not inspected in detail but consistent with the known pattern) across three consecutive default-sampling runs in this session. Applying the same `--override-generation-config '{"temperature": 0.2, "repetition_penalty": 1.1}'` fix already used for the Qwen3.6 profile fixed it cleanly — 10/10 pass@1, no throughput cost, and concurrent-8 even edged up slightly (likely noise, not a real effect).

**Final adopted config:** `--dtype float16 --enforce-eager --tensor-parallel-size 2 --max-model-len 16384 --max-num-seqs 16 --gpu-memory-utilization 0.90 --enable-prefix-caching --enable-chunked-prefill --max-num-batched-tokens 16384 --mamba-ssm-cache-dtype float16 --reasoning-parser qwen3 --enable-auto-tool-choice --tool-call-parser qwen3_coder --override-generation-config '{"temperature": 0.2, "repetition_penalty": 1.1}'`.

**Switch-over: `docker-compose.yml`'s `vllm-qwen36moe` service updated** — model path and `--served-model-name` changed to `Huihui-Qwen3.8-27B-abliterated`/`qwen3.8-27b`, `--quantization ascend` removed (bf16-cast-to-fp16, not a msmodelslim checkpoint), ACLGraph `--additional-config`/`--compilation-config` flags removed and replaced with `--enforce-eager` (mandatory for this architecture — see the two sections above), `--max-model-len` set to 16384 per the user's explicit request, prefix-caching/chunked-prefill flags added. `--mamba-ssm-cache-dtype float16` kept — confirmed still required (this model has `linear_attention` layers, i.e. gated-delta-net blocks, same as the MoE Qwen3.6 model that needed this flag; it is not MoE-specific as might otherwise be assumed). Verified end-to-end via Compose: `docker compose --profile qwen36 up -d --force-recreate` → `Application startup complete` → `/v1/models` shows `qwen3.8-27b` / `max_model_len: 16384` → live `is_prime` request returns correct code, `finish_reason: stop`.

**Not yet done:** no soak test of this config under real multi-turn Open WebUI usage (only the standard bench harness + one manual canary); the `roman`-failure investigation (reasoning-non-termination vs. genuine logic error) was made moot by the sampling fix rather than root-caused; concurrent throughput beyond 8 concurrent requests untested.

**Test method:** production (`qwen36` profile, previously the w8a8 Qwen3.6 model) stopped for the full tuning pass, four configs tested standalone on port 8003 (each a fresh container), full `bench/run_bench.py` suite run per config. After settling on the final config, `docker-compose.yml` was updated directly (not tested as a separate standalone step first, since every component of the final config had already been individually validated standalone) and brought up via `docker compose --profile qwen36 up -d --force-recreate` — this **is** the new production state, not a reverted experiment. The prior w8a8 Qwen3.6 weights remain on disk, untouched, for rollback (see "Production Setup" table above).
